<template>
  <div
    class="bg-bg-bg-innerframe gap-size-8 mb-3 flex flex-col items-center justify-center rounded-md p-4"
  >
    <span class="text-font-14 font-normal"> 인증번호 전송 이메일 주소 </span>
    <span class="text-font-18 font-semibold"> {{ email }} </span>
  </div>
  <div>
...
const handleClickNext = async () => {
  try {
    const res = await authService.verifyEmailVerificationCode({
      email: email.value,
      verificationCode: inputValues.value.join(''),
      useType: 'RESET_PASSWORD',
    });
